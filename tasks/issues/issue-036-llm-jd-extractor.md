...
- [ ] 新增 `backend/infrastructure/extractors/jd_extractor.py`：`JDExtractor.extract(raw_text) -> JDExtraction`
- [ ] 输出结构含 required_skills / critical_skills / responsibilities / seniority（junior/mid/senior/expert 四档），技能项带原文 evidence
- [ ] LLM 输出不合法（JSON 解析或 schema 校验失败）→ 内部重试 1 次；仍失败 → 抛 `JDExtractionError`
- [ ] Prompt 与解析逻辑与 `llm_extractor.py` 风格一致（结构化输出 + pydantic 校验）
- [ ] 单测：正常抽取 / 输出畸形重试成功 / 重试仍失败三路径（mock LLMGateway）
- [ ] Lint / mypy 通过

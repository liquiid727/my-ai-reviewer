...
- [ ] 上传控件限制 jpg/png、≤10MB，前端先行校验并提示
- [ ] 背景色选择（white/blue/red），调用 `POST /{draft_id}/photo`
- [ ] 处理结果原图/成品对比预览；降级（`background_replaced=false`）时展示 `degraded_reason` 提示
- [ ] 确认采用（PUT confirm）后简历预览区显示头像；支持移除（DELETE）与重新上传
- [ ] 四态齐全：空（未上传）/ 加载（处理中）/ 成功 / 失败（400/422/501 差异化文案）
- [ ] 浏览器实际验证完整流程（上传→预览→确认→导出 PDF 含照片）
- [ ] Lint（oxlint）/ tsc 通过

...
class JobDescriptionModel(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "processing_step IN ('queued', 'source_validate', 'source_extract', 'vision_extract', "
            "'text_quality_check', 'duplicate_check', 'structure_parse', 'llm_extract', 'review', 'done')",
...
class JDSourceAssetModel(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('stored', 'transcribing', 'ready', 'failed', 'deleted')",
            name="ck_jd_asset_status",
        ),
...
class JDMatchResultModel(Base):
    ...
    matcher_version: Mapped[str] = mapped_column(
        String(80), nullable=False, default="rules-v1", server_default="rules-v1"
    )

...
def jd_source_validate_task(jd_id_str: str, run_id_str: str) -> str:
    return _run_async(_run_stage(JDProcessingStep.SOURCE_VALIDATE.value, uuid.UUID(jd_id_str), uuid.UUID(run_id_str)))
...
def jd_vision_extract_task(self: Any, previous: str, jd_id_str: str, run_id_str: str) -> str:
    ...
    try:
        ...
        if self.request.retries >= (self.max_retries or 0):
            return _run_async(_mark_stage_failed(JDProcessingStep.VISION_EXTRACT.value, jd_id, run_id, safe_error))

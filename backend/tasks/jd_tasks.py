...
from backend.domain.jd.services import JDProcessingError, JDProcessingService
        ...
        try:
            ...
            if stage == JDProcessingStep.DUPLICATE_CHECK.value:
                return await service.duplicate_check(
                    session, jd_id, run_id, allow_duplicate=allow_duplicate,
                )
...
def jd_source_extract_task(jd_id_str: str, run_id_str: str) -> str:
    return asyncio.run(
        _run_stage(JDProcessingStep.SOURCE_EXTRACT.value, uuid.UUID(jd_id_str), uuid.UUID(run_id_str))
    )
...
def jd_llm_extract_task(
    ...
    try:
        ...
        safe_error = exc if isinstance(exc, JDProcessingError) else JDProcessingError(
            "JD structured extraction failed",
            code=5001,
        )
        if self.request.retries >= (self.max_retries or 0):
            return asyncio.run(
                _mark_stage_failed(JDProcessingStep.LLM_EXTRACT.value, jd_id, run_id, safe_error)
            )

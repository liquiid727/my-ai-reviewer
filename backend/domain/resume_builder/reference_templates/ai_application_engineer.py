"""AI 应用开发工程师参考模板 —— 结构化草稿构建器。

内容与同目录 ai_application_engineer.md（人类可读范本）保持一致；
占位符（如 [姓名]）保留，供用户在编辑器中替换为真实信息。
"""
...
_SUMMARY = (
    "拥有 3 年以上 Go 后端与平台工程经验，具备复杂业务系统、分布式服务、资源调度、"
    "支付计费和可观测性建设经验。当前重点转向 AI 应用开发，围绕 LLM API、Structured Output、"
    "Function Calling、MCP、RAG、Workflow、Agent、上下文工程和评测体系进行系统学习与项目实践。"
    "能够将 AI 能力接入现有业务系统，完成从需求分析、模型接入、知识库检索、工具调用、"
    "服务编排到部署监控的完整链路。熟悉 Go 服务端开发，同时具备 Python、FastAPI、React "
    "和 AI Coding 工具链实践经验，适合承担「后端工程能力 + AI 产品落地」的复合型岗位。"
)


def build_ai_application_engineer_draft() -> ResumeDraft:
    """构建 AI 应用开发工程师范本草稿（每次调用返回全新对象）。"""
    sections = [
        DraftSection(
        ...
        DraftSection(
            ...
            items=[
                DraftItem(
                    heading="某科技公司",
                    subheading="Go 后端开发工程师",
                    date_range="2023.01 - 至今",
                    bullets=[
        ...
        DraftSection(
            ...
            items=[
                DraftItem(
                    heading="企业级 AI Gateway 与模型服务平台",
                    subheading="核心开发 / 架构设计｜个人项目 / 二次开发",
                    bullets=[
                ...
                DraftItem(
                    heading="企业知识库与智能客服应用",
                    subheading="AI 应用开发｜个人项目 / 作品集",
                    bullets=[
                ...
                DraftItem(
                    heading="云游戏资源调度与会话平台",
                    subheading="Go 后端核心开发｜工作项目",
                    bullets=[
                ...
                DraftItem(
                    heading="SpecOS AI 研发工作流",
                    subheading="产品设计 / 全栈开发｜个人项目 / 研发中",
                    bullets=[
    ...
    return ResumeDraft(
        title="AI 应用开发工程师（参考模板）",
        identity={
            "name": "[姓名]",
            "phone": "[手机号码]",
            "email": "[邮箱地址]",
            "location": "[当前城市]",
            "links": ["[GitHub 地址]", "[博客地址]"],
        },
        summary=_SUMMARY,
        sections=sections,
